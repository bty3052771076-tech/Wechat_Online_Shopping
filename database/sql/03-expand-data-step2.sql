-- ============================================================
-- 商品数据扩充 - 续跑脚本（Step 2/3）
-- 前置: 03-expand-data.sql 的 1-A/1-B 已执行成功，分类共 20 条
-- 本脚本: 1-C 新增子分类 + 全部 SPU + 全部 SKU
-- ============================================================
...
-- ============================================================
-- 1-C: 新增食品饮料/数码家电/母婴用品的二级分类
-- ============================================================
SET @food_id    = (SELECT id FROM categories WHERE category_code='CATEGORY_FOOD'    LIMIT 1);
SET @digital_id = (SELECT id FROM categories WHERE category_code='CATEGORY_DIGITAL' LIMIT 1);
SET @baby_id    = (SELECT id FROM categories WHERE category_code='CATEGORY_BABY'    LIMIT 1);
...
('SPU075', '卫龙大面筋辣条礼包', '香辣过瘾 童年回味', (SELECT id FROM categories WHERE category_code='FOOD_SNACK'), '卫龙', '/assets/images/products/spicy-strip.png', '["/assets/images/products/spicy-strip.png"]', '<p>卫龙辣条经典系列，选用优质面筋，独立小包装，健康卫生。</p>', 39.90, 59.00, 300, 267, '热销,辣条', 1, NOW(), NOW()),
...
-- -------- 手机配件（SPU090～SPU094）--------
('SPU090', '苹果原装20W快充套装', '官方原装 安全快速', (SELECT id FROM categories WHERE category_code='DIGITAL_PHONE_ACC'), 'Apple', '/assets/images/products/apple-charger.png', '["/assets/images/products/apple-charger.png"]', '<p>苹果原装20W USB-C快充充电器，配套1m数据线，安全可靠。</p>', 149.00, 199.00, 150, 89, '原装,快充', 1, NOW(), NOW()),
...
-- -------- 婴儿食品（SPU099～SPU102）--------
('SPU099', '英雄有机高铁米粉', '补铁强化 助力成长', (SELECT id FROM categories WHERE category_code='BABY_FOOD'), '英雄', '/assets/images/products/baby-rice.png', '["/assets/images/products/baby-rice.png"]', '<p>英雄有机婴儿营养米粉，强化铁质，有机认证，适合6个月以上宝宝。</p>', 49.90, 79.00, 200, 123, '婴儿辅食,有机', 1, NOW(), NOW()),
...
SELECT CONCAT('SKU总数(status=1): ', COUNT(*)) AS result FROM product_skus WHERE status=1;
