...
import { addAddress, updateAddress } from '../../../../services/address-new';

const { buildAddressPayload, adaptAddressRecord } = require('../../../../services/_utils/shop-adapters');
const { validateAddressForm } = require('../../../../services/_utils/form-helpers');

const innerPhoneReg = '^1(?:3\\d|4[4-9]|5[0-35-9]|6[67]|7[0-8]|8\\d|9\\d)\\d{8}$';
const innerNameReg = '^[a-zA-Z\\d\\u4e00-\\u9fa5\\s]+$';
const labelsOptions = [
  { id: 0, name: '家' },
  { id: 1, name: '公司' },
];
...
Page({
  ...
  onVerifyInputLegal() {
    ...
    return validateAddressForm(this.data.locationState, {
      phoneReg: properties.phoneReg || innerPhoneReg,
      nameReg: properties.nameReg || innerNameReg,
    });
  },
      ...
      wx.chooseLocation({
        success: (res) => {
          if (res.name) {
            this.setData(
              {
                'locationState.detailAddress': res.address || res.name,
  ...
  formSubmit() {
    ...
    const payload = buildAddressPayload(locationState);
    ...
    request
      .then((res) => {
        this.hasSava = true;
        resolveAddress(adaptAddressRecord(res.data));
