...
Page({
  ...
  getService() {
    return getRightsDetail({ rightsNo: this.rightsNo }).then((res) => {
      ...
      const service = buildAfterServiceDetailViewModel(serviceRaw, {
        formatTime,
        getStatusIcon: (rights) => this.genStatusIcon(rights),
        serviceTypeDesc: ServiceTypeDesc,
        refundedStatus: ServiceStatus.REFUNDED,
      });
      ...
      this.setData({
        ...
        service,
        deliveryButton: {},
        'gallery.proofs': service.proofs,
        showProofs:
          serviceRaw.rights.userRightsStatus === ServiceStatus.PENDING_VERIFY &&
          Boolean(service.applyRemark || service.proofs.length > 0),
      });

      wx.setNavigationBarTitle({
        title: TitleConfig[service.type] || '售后详情',
      });
    });
  },
