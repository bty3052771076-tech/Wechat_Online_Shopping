...
Page({
  ...
  handleOptionsParams(options, couponList) {
    ...
    if (options.type === 'cart') {
      ...
      goodsRequestList = parseGoodsRequestListParam(options.goodsRequestList);
    }
    //获取结算页请求数据列表
    const storeMap = {};
  ...
  handlePay(data, settleDetailData) {
    ...
    wx.redirectTo({
      url: `/pages/order/pay-result/index?totalPaid=${totalPayAmount}&orderNo=${tradeNo}&mode=submit`,
    });
  },
